...
def export_tsv(infile, outfile, format, outcsv, transition_quantification, ipf, max_rs_peakgroup_qvalue, peptide, max_global_peptide_qvalue, protein, max_global_protein_qvalue):
    ...
    if ipf_present and ipf:
        if transition_quantification:
            data = pd.read_sql_query("SELECT RUN.ID AS id_run, PEPTIDE.ID AS id_peptide, PEPTIDE_IPF.MODIFIED_SEQUENCE || '_' || PRECURSOR.ID AS transition_group_id, PRECURSOR.DECOY AS decoy, RUN.ID AS run_id, RUN.FILENAME AS filename, FEATURE.EXP_RT AS RT, FEATURE.EXP_RT - FEATURE.DELTA_RT AS library_RT, FEATURE.DELTA_RT AS delta_rt, FEATURE.NORM_RT AS iRT, PRECURSOR.LIBRARY_RT AS library_iRT, FEATURE.NORM_RT - PRECURSOR.LIBRARY_RT AS delta_iRT, FEATURE.ID AS id, PEPTIDE_IPF.UNMODIFIED_SEQUENCE AS Sequence, PEPTIDE_IPF.MODIFIED_SEQUENCE AS FullUniModPeptideName, PRECURSOR.CHARGE AS Charge, PRECURSOR.PRECURSOR_MZ AS mz, FEATURE_MS2.AREA_INTENSITY AS Intensity, FEATURE_MS1.AREA_INTENSITY AS aggr_prec_peak_area, FEATURE_MS1.APEX_INTENSITY AS aggr_prec_peak_apex, aggr_peak_area, aggr_peak_apex, aggr_fragment_annotation, PRECURSOR.LIBRARY_RT AS assay_rt, FEATURE.LEFT_WIDTH AS leftWidth, FEATURE.RIGHT_WIDTH AS rightWidth, SCORE_MS1.PEP AS ms1_pep, SCORE_MS2.PEP AS ms2_pep, SCORE_IPF.PRECURSOR_PEAKGROUP_PEP AS precursor_pep, SCORE_IPF.PEP AS ipf_pep, SCORE_MS2.RANK AS peak_group_rank, SCORE_MS2.SCORE AS d_score, SCORE_MS2.QVALUE AS ms2_m_score, SCORE_IPF.QVALUE AS m_score FROM PRECURSOR JOIN PRECURSOR_PEPTIDE_MAPPING ON PRECURSOR.ID = PRECURSOR_PEPTIDE_MAPPING.PRECURSOR_ID JOIN PEPTIDE ON PRECURSOR_PEPTIDE_MAPPING.PEPTIDE_ID = PEPTIDE.ID INNER JOIN FEATURE ON FEATURE.PRECURSOR_ID = PRECURSOR.ID JOIN RUN ON RUN.ID = FEATURE.RUN_ID JOIN FEATURE_MS1 ON FEATURE_MS1.FEATURE_ID = FEATURE.ID JOIN FEATURE_MS2 ON FEATURE_MS2.FEATURE_ID = FEATURE.ID JOIN (SELECT FEATURE_ID, GROUP_CONCAT(AREA_INTENSITY,';') AS aggr_peak_area, GROUP_CONCAT(APEX_INTENSITY,';') AS aggr_peak_apex, GROUP_CONCAT(TRANSITION_ID,';') AS aggr_fragment_annotation FROM FEATURE_TRANSITION INNER JOIN TRANSITION ON FEATURE_TRANSITION.TRANSITION_ID = TRANSITION.ID WHERE TRANSITION.DECOY == 0 GROUP BY FEATURE_ID) AS FEATURE_TRANSITION ON FEATURE_TRANSITION.FEATURE_ID = FEATURE.ID JOIN SCORE_MS1 ON SCORE_MS1.FEATURE_ID = FEATURE.ID JOIN SCORE_MS2 ON SCORE_MS2.FEATURE_ID = FEATURE.ID JOIN SCORE_IPF ON SCORE_IPF.FEATURE_ID = FEATURE.ID JOIN PEPTIDE AS PEPTIDE_IPF ON SCORE_IPF.PEPTIDE_ID = PEPTIDE_IPF.ID;", con)
        else:
            data = pd.read_sql_query("SELECT RUN.ID AS id_run, PEPTIDE.ID AS id_peptide, PEPTIDE_IPF.MODIFIED_SEQUENCE || '_' || PRECURSOR.ID AS transition_group_id, PRECURSOR.DECOY AS decoy, RUN.ID AS run_id, RUN.FILENAME AS filename, FEATURE.EXP_RT AS RT, FEATURE.EXP_RT - FEATURE.DELTA_RT AS library_RT, FEATURE.DELTA_RT AS delta_rt, FEATURE.NORM_RT AS iRT, PRECURSOR.LIBRARY_RT AS library_iRT, FEATURE.NORM_RT - PRECURSOR.LIBRARY_RT AS delta_iRT, FEATURE.ID AS id, PEPTIDE_IPF.UNMODIFIED_SEQUENCE AS Sequence, PEPTIDE_IPF.MODIFIED_SEQUENCE AS FullUniModPeptideName, PRECURSOR.CHARGE AS Charge, PRECURSOR.PRECURSOR_MZ AS mz, FEATURE_MS2.AREA_INTENSITY AS Intensity, FEATURE_MS1.AREA_INTENSITY AS aggr_prec_peak_area, FEATURE_MS1.APEX_INTENSITY AS aggr_prec_peak_apex, PRECURSOR.LIBRARY_RT AS assay_rt, FEATURE.LEFT_WIDTH AS leftWidth, FEATURE.RIGHT_WIDTH AS rightWidth, SCORE_MS1.PEP AS ms1_pep, SCORE_MS2.PEP AS ms2_pep, SCORE_IPF.PRECURSOR_PEAKGROUP_PEP AS precursor_pep, SCORE_IPF.PEP AS ipf_pep, SCORE_MS2.RANK AS peak_group_rank, SCORE_MS2.SCORE AS d_score, SCORE_MS2.QVALUE AS ms2_m_score, SCORE_IPF.QVALUE AS m_score FROM PRECURSOR JOIN PRECURSOR_PEPTIDE_MAPPING ON PRECURSOR.ID = PRECURSOR_PEPTIDE_MAPPING.PRECURSOR_ID JOIN PEPTIDE ON PRECURSOR_PEPTIDE_MAPPING.PEPTIDE_ID = PEPTIDE.ID INNER JOIN FEATURE ON FEATURE.PRECURSOR_ID = PRECURSOR.ID JOIN RUN ON RUN.ID = FEATURE.RUN_ID JOIN FEATURE_MS1 ON FEATURE_MS1.FEATURE_ID = FEATURE.ID JOIN FEATURE_MS2 ON FEATURE_MS2.FEATURE_ID = FEATURE.ID JOIN SCORE_MS1 ON SCORE_MS1.FEATURE_ID = FEATURE.ID JOIN SCORE_MS2 ON SCORE_MS2.FEATURE_ID = FEATURE.ID JOIN SCORE_IPF ON SCORE_IPF.FEATURE_ID = FEATURE.ID JOIN PEPTIDE AS PEPTIDE_IPF ON SCORE_IPF.PEPTIDE_ID = PEPTIDE_IPF.ID;", con)

    else:
        if transition_quantification:
            data = pd.read_sql_query("SELECT RUN.ID AS id_run, PEPTIDE.ID AS id_peptide, PRECURSOR.ID AS transition_group_id, PRECURSOR.DECOY AS decoy, RUN.ID AS run_id, RUN.FILENAME AS filename, FEATURE.EXP_RT AS RT, FEATURE.EXP_RT - FEATURE.DELTA_RT AS library_RT, FEATURE.DELTA_RT AS delta_rt, FEATURE.NORM_RT AS iRT, PRECURSOR.LIBRARY_RT AS library_iRT, FEATURE.NORM_RT - PRECURSOR.LIBRARY_RT AS delta_iRT, FEATURE.ID AS id, PEPTIDE.UNMODIFIED_SEQUENCE AS Sequence, PEPTIDE.MODIFIED_SEQUENCE AS FullUniModPeptideName, PRECURSOR.CHARGE AS Charge, PRECURSOR.PRECURSOR_MZ AS mz, FEATURE_MS2.AREA_INTENSITY AS Intensity, FEATURE_MS1.AREA_INTENSITY AS aggr_prec_peak_area, FEATURE_MS1.APEX_INTENSITY AS aggr_prec_peak_apex, aggr_peak_area, aggr_peak_apex, aggr_fragment_annotation, PRECURSOR.LIBRARY_RT AS assay_rt, FEATURE.LEFT_WIDTH AS leftWidth, FEATURE.RIGHT_WIDTH AS rightWidth, SCORE_MS2.RANK AS peak_group_rank, SCORE_MS2.SCORE AS d_score, SCORE_MS2.QVALUE AS m_score FROM PRECURSOR JOIN PRECURSOR_PEPTIDE_MAPPING ON PRECURSOR.ID = PRECURSOR_PEPTIDE_MAPPING.PRECURSOR_ID JOIN PEPTIDE ON PRECURSOR_PEPTIDE_MAPPING.PEPTIDE_ID = PEPTIDE.ID INNER JOIN FEATURE ON FEATURE.PRECURSOR_ID = PRECURSOR.ID JOIN RUN ON RUN.ID = FEATURE.RUN_ID JOIN FEATURE_MS1 ON FEATURE_MS1.FEATURE_ID = FEATURE.ID JOIN FEATURE_MS2 ON FEATURE_MS2.FEATURE_ID = FEATURE.ID JOIN (SELECT FEATURE_ID, GROUP_CONCAT(AREA_INTENSITY,';') AS aggr_peak_area, GROUP_CONCAT(APEX_INTENSITY,';') AS aggr_peak_apex, GROUP_CONCAT(TRANSITION_ID,';') AS aggr_fragment_annotation FROM FEATURE_TRANSITION INNER JOIN TRANSITION ON FEATURE_TRANSITION.TRANSITION_ID = TRANSITION.ID GROUP BY FEATURE_ID) AS FEATURE_TRANSITION ON FEATURE_TRANSITION.FEATURE_ID = FEATURE.ID JOIN SCORE_MS2 ON SCORE_MS2.FEATURE_ID = FEATURE.ID;", con)
        else:
            data = pd.read_sql_query("SELECT RUN.ID AS id_run, PEPTIDE.ID AS id_peptide, PRECURSOR.ID AS transition_group_id, PRECURSOR.DECOY AS decoy, RUN.ID AS run_id, RUN.FILENAME AS filename, FEATURE.EXP_RT AS RT, FEATURE.EXP_RT - FEATURE.DELTA_RT AS library_RT, FEATURE.DELTA_RT AS delta_rt, FEATURE.NORM_RT AS iRT, PRECURSOR.LIBRARY_RT AS library_iRT, FEATURE.NORM_RT - PRECURSOR.LIBRARY_RT AS delta_iRT, FEATURE.ID AS id, PEPTIDE.UNMODIFIED_SEQUENCE AS Sequence, PEPTIDE.MODIFIED_SEQUENCE AS FullUniModPeptideName, PRECURSOR.CHARGE AS Charge, PRECURSOR.PRECURSOR_MZ AS mz, FEATURE_MS2.AREA_INTENSITY AS Intensity, FEATURE_MS1.AREA_INTENSITY AS aggr_prec_peak_area, FEATURE_MS1.APEX_INTENSITY AS aggr_prec_peak_apex, PRECURSOR.LIBRARY_RT AS assay_rt, FEATURE.LEFT_WIDTH AS leftWidth, FEATURE.RIGHT_WIDTH AS rightWidth, SCORE_MS2.RANK AS peak_group_rank, SCORE_MS2.SCORE AS d_score, SCORE_MS2.QVALUE AS m_score FROM PRECURSOR JOIN PRECURSOR_PEPTIDE_MAPPING ON PRECURSOR.ID = PRECURSOR_PEPTIDE_MAPPING.PRECURSOR_ID JOIN PEPTIDE ON PRECURSOR_PEPTIDE_MAPPING.PEPTIDE_ID = PEPTIDE.ID INNER JOIN FEATURE ON FEATURE.PRECURSOR_ID = PRECURSOR.ID JOIN RUN ON RUN.ID = FEATURE.RUN_ID JOIN FEATURE_MS1 ON FEATURE_MS1.FEATURE_ID = FEATURE.ID JOIN FEATURE_MS2 ON FEATURE_MS2.FEATURE_ID = FEATURE.ID JOIN SCORE_MS2 ON SCORE_MS2.FEATURE_ID = FEATURE.ID;", con)
