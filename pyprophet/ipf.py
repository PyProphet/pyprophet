...
def read_pyp_peakgroup_precursor(
    path, ipf_max_peakgroup_pep, ipf_ms1_scoring, ipf_ms2_scoring
):
    ...
    if not ipf_ms1_scoring and ipf_ms2_scoring:
        if not check_sqlite_table(con_sqlite, "SCORE_MS2") or not check_sqlite_table(
            con_sqlite, "SCORE_TRANSITION"
        ):
        ...
        create_index_if_not_exists(
            con, "idx_feature_precursor_id", "FEATURE", "PRECURSOR_ID"
        )
        create_index_if_not_exists(con, "idx_feature_feature_id", "FEATURE", "ID")
        create_index_if_not_exists(
            con, "idx_score_ms2_feature_id", "SCORE_MS2", "FEATURE_ID"
        )
        create_index_if_not_exists(
            con, "idx_score_transition_feature_id", "SCORE_TRANSITION", "FEATURE_ID"
        )
        create_index_if_not_exists(
            con,
            "idx_score_transition_transition_id",
            "SCORE_TRANSITION",
            "TRANSITION_ID",
        )
...
""",
    ...
    # only use MS1 precursors
        ...
        if (
            not check_sqlite_table(con_sqlite, "SCORE_MS1")
            or not check_sqlite_table(con_sqlite, "SCORE_MS2")
            or not check_sqlite_table(con_sqlite, "SCORE_TRANSITION")
        ):
            raise click.ClickException(
        ...
        create_index_if_not_exists(con, "idx_feature_feature_id", "FEATURE", "ID")
        create_index_if_not_exists(
            con, "idx_score_ms1_feature_id", "SCORE_MS1", "FEATURE_ID"
        )
        create_index_if_not_exists(
            con, "idx_score_ms2_feature_id", "SCORE_MS2", "FEATURE_ID"
        )
...
def read_pyp_transition(path, ipf_max_transition_pep, ipf_h0):
    ...
    create_index_if_not_exists(
        con,
        "idx_transition_peptide_mapping_transition_id",
    ...
    create_index_if_not_exists(
        con, "idx_score_transition_transition_id", "SCORE_TRANSITION", "TRANSITION_ID"
    )
...
def read_pyp_parquet_peakgroup_precursor(
    path, ipf_max_peakgroup_pep, ipf_ms1_scoring, ipf_ms2_scoring
):
    ...
    if not ipf_ms1_scoring and ipf_ms2_scoring:
        if not any(
            [col.startswith("SCORE_MS2_") for col in all_column_names]
        ) or not any([col.startswith("SCORE_TRANSITION_") for col in all_column_names]):
            raise click.ClickException(
                "Apply scoring to MS2 and transition-level data before running IPF."
            )
        ...
        data = (
            data.explode(
                ["TRANSITION_TYPE", "TRANSITION_DECOY", "SCORE_TRANSITION_PEP"]
            )
            .filter(
            ...
            .with_columns(pl.lit(None).cast(pl.Float64).alias("MS1_PRECURSOR_PEP"))
    ...
    # only use MS1 precursors
        ...
        if (
            not any([col.startswith("SCORE_MS1_") for col in all_column_names])
            or not any([col.startswith("SCORE_MS2_") for col in all_column_names])
            or not any(
                [col.startswith("SCORE_TRANSITION_") for col in all_column_names]
            )
        ):
            raise click.ClickException(
                "Apply scoring to MS1, MS2 and transition-level data before running IPF."
        ...
        data = (
            ...
            .with_columns(pl.lit(None).cast(pl.Float64).alias("MS2_PRECURSOR_PEP"))
    ...
    # do not use any precursor information
        ...
        data = (
            ...
            .rename({"SCORE_MS2_PEP": "MS2_PEAKGROUP_PEP"})
            .with_columns(
...
def read_pyp_parquet_transition(path, ipf_max_transition_pep, ipf_h0):
    ...
    bitmask = (
        bitmask.explode(
            ["TRANSITION_ID", "PEPTIDE_IPF_ID", "TRANSITION_TYPE", "TRANSITION_DECOY"]
        )
        .filter(
            (pl.col("TRANSITION_TYPE") != "")
            & (pl.col("TRANSITION_DECOY") == 0)
        ...
        .drop(["TRANSITION_TYPE", "TRANSITION_DECOY"])
        .select(["TRANSITION_ID", "PEPTIDE_ID", "BMASK"])
        .unique()
        .to_pandas()
    )
    bitmask.columns = [col.lower() for col in bitmask.columns]
    ...
    num_peptidoforms = (
        num_peptidoforms.explode(
            ["PEPTIDE_IPF_ID", "TRANSITION_TYPE", "TRANSITION_DECOY"]
        )
        .filter(
            (pl.col("TRANSITION_TYPE") != "")
            & (pl.col("TRANSITION_DECOY") == 0)
        ...
        .sort("FEATURE_ID")
        .select(["FEATURE_ID", "NUM_PEPTIDOFORMS"])
        .to_pandas()
    )
    num_peptidoforms.columns = [col.lower() for col in num_peptidoforms.columns]
...
def read_pyp_parquet_dir_peakgroup_precursor(
    path, ipf_max_peakgroup_pep, ipf_ms1_scoring, ipf_ms2_scoring
):
    click.echo("Info: Reading precursor-level data ... ", nl=False)
    # precursors are restricted according to ipf_max_peakgroup_pep to exclude very poor peak groups
    ...
    if not ipf_ms1_scoring and ipf_ms2_scoring:
        if not any(
            ...
            [col.startswith("SCORE_TRANSITION_") for col in all_transition_column_names]
        ):
            raise click.ClickException(
                "Apply scoring to MS2 and transition-level data before running IPF."
            )
    ...
    # only use MS1 precursors
        ...
        if not any(
            [col.startswith("SCORE_MS1_") for col in all_precursor_column_names]
        ) or not any(
            [col.startswith("SCORE_MS2_") for col in all_precursor_column_names]
        ):
            raise click.ClickException(
    ...
    # use both MS1 and MS2 precursors
        ...
        if (
            ...
            or not any(
                [
                    col.startswith("SCORE_TRANSITION_")
...
def infer_peptidoforms(
    ...
    if is_parquet_file(infile):
        ...
        precursor_table = read_pyp_peakgroup_precursor(
            infile, ipf_max_peakgroup_pep, ipf_ms1_scoring, ipf_ms2_scoring
        )
    ...
    if is_parquet_file(infile):
        ...
        sub_init_df = (
            init_df.select(
                ["PEPTIDE_ID", "PRECURSOR_ID", "PEPTIDE_IPF_ID", "FEATURE_ID"]
            )
        ...
        peptidoform_data_m = init_df.join(
            peptidoform_data_m,
        ...
        peptidoform_data = peptidoform_data.rename(
            columns={
                "PRECURSOR_PEAKGROUP_PEP": "SCORE_IPF_PRECURSOR_PEAKGROUP_PEP",
                "QVALUE": "SCORE_IPF_QVALUE",
                "PEP": "SCORE_IPF_PEP",
