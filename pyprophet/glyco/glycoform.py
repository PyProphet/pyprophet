...
def read_glycoform_space(path, use_glycan_composition):
    ...
    if not check_sqlite_table(con, "SCORE_TRANSITION"):
        raise click.ClickException(
            "Apply scoring to transition-level data before running glycoform inference."
        )
...
""",
    ...
    glycoforms = pd.concat([glycoforms, h0], sort=True)

    data = pd.merge(glycoforms, num_glycoforms, how="left", on="feature_id")

    if use_glycan_composition:
        data["num_glycan_composition"] = data.groupby(["feature_id"])[
            "glycan_composition"
        ].transform(lambda x: len(x[x != ""].unique()))
        data["num_glycan_struct"] = data.groupby(["feature_id", "glycan_composition"])[
            "glycan_struct"
...
def read_ms2_peakgroup(path, max_peakgroup_pep):
    ...
    if not check_sqlite_table(con, "SCORE_MS2"):
        raise click.ClickException(
            "Apply scoring to MS2-level data before running glycoform inference."
        )
...
def read_ms1_precursor(path, max_precursor_pep):
    ...
    if not check_sqlite_table(con, "SCORE_MS1"):
        raise click.ClickException(
            "Apply scoring to MS1-level data before running glycoform inference."
        )
...
def calculate_ms2_peakgroup_prior(data, use_glycan_composition):
    if use_glycan_composition:
        return data.apply(
            lambda x: (
                (1 - x["ms2_peakgroup_pep"])
                / x["num_glycan_composition"]
                / x["num_glycan_struct"]
                if x["glycopeptide_id"] != -1
                else x["ms2_peakgroup_pep"]
            ),
            axis=1,
        )
    else:
        return data.apply(
            lambda x: (
                (1 - x["ms2_peakgroup_pep"]) / x["num_glycoforms"]
                if x["glycopeptide_id"] != -1
                else x["ms2_peakgroup_pep"]
            ),
            axis=1,
...
def prepare_ms1_precursor_bm(
    ...
    if ms1_mz_window_unit == "ppm":
        ...
        def match_precursor_mz(
            precursor_mz, precursor_charge, glycan_mw_1, glycan_mw_2
        ):
            return (
                abs(glycan_mw_2 - glycan_mw_1) / (precursor_charge * precursor_mz)
                <= ms1_mz_window * 1e-6
            )
        ...
        def match_precursor_mz(
            ...
            return abs(glycan_mw_2 - glycan_mw_1) / precursor_charge <= ms1_mz_window

    else:
        raise ValueError(
            "invalid m/z extraction window unit: " + str(ms1_mz_window_unit)
        )
    ...
    data["prior"] = calculate_ms2_peakgroup_prior(
        data, use_glycan_composition=use_glycan_composition
    )

    data["match_precursor"] = data.apply(
        lambda x: np.isnan(x["ms1_precursor_pep"])
        or (x["glycopeptide_id"] != -1)
        and match_precursor_mz(
            x["precursor_mz"],
            x["precursor_charge"],
    ...
    data["evidence"] = 1 - data["ms1_precursor_pep"].fillna(1)
    ...
    return data[
        ["feature_id", "num_glycoforms", "prior", "evidence", "glycopeptide_id"]
    ].rename(columns={"glycopeptide_id": "hypothesis"})


def prepare_ms2_precursor_bm(
    glycoforms, ms2_precursor_table, ms2_peakgroup_table, use_glycan_composition
):
    ...
    data["evidence"] = data.apply(
        lambda x: (
            1 - x["ms2_precursor_pep"] if x["bmask"] == 1 else x["ms2_precursor_pep"]
        ),
        axis=1,
    )
...
def prepare_transition_bm(
    ...
    if propagate_signal_across_runs:
        ...
        non_prop_data = evidence.loc[
            evidence["feature_id"] == evidence["alignment_group_id"]
        ]
        prop_data = evidence.loc[
            evidence["feature_id"] != evidence["alignment_group_id"]
        ]
    ...
    bitmask = transition_table[["transition_id", "glycopeptide_id"]].drop_duplicates()
    bitmask["bmask"] = 1
    data = pd.merge(data, bitmask, how="left", on=["transition_id", "glycopeptide_id"])
    ...
    data["evidence"] = data.apply(
        lambda x: 1 - x["pep"] if x["bmask"] == 1 else x["pep"], axis=1
    )
...
def infer_glycoforms(
    ...
    glycoforms = read_glycoform_space(
        infile, use_glycan_composition=use_glycan_composition
    )
    ...
    if ms1_precursor_scoring:
        ms1_precursor_table = read_ms1_precursor(
            infile, max_precursor_pep=max_precursor_pep
        )
    ...
    if ms2_precursor_scoring:
        ms2_precursor_table = read_ms2_precursor(infile, max_precursor_pep)
    ...
    if ms1_precursor_scoring and ms2_precursor_scoring:
        precursor_bm_data = pd.concat([ms1_precursor_bm_data, ms2_precursor_bm_data])
    ...
    if precursor_bm_data is not None:
        ...
        precursor_peakgroup_data = prec_pp_data.rename(
            columns={"hypothesis": "glycopeptide_id"}
        )[["feature_id", "glycopeptide_id", "precursor_peakgroup_pep"]]
    else:
        precursor_peakgroup_data = pd.merge(
            glycoforms, ms2_peakgroup_table, how="inner", on="feature_id"
        )
        precursor_peakgroup_data["precursor_peakgroup_pep"] = (
            calculate_ms2_peakgroup_prior(
                precursor_peakgroup_data, use_glycan_composition=use_glycan_composition
            )
        )
    ...
    transition_table = read_transition(infile, max_transition_pep)
    ...
    if propagate_signal_across_runs:
        ...
        transition_table["alignment_group_id"] = transition_table[
            "alignment_group_id"
        ].astype(object)
        mask = transition_table["alignment_group_id"].isna()
        transition_table.loc[mask, "alignment_group_id"] = transition_table.loc[
            mask, "feature_id"
        ].astype(str)
    ...
    if use_glycan_composition:
        glycoform_pp_data = pd.merge(
            glycoform_pp_data,
            glycoforms[["feature_id", "glycopeptide_id", "glycan_composition"]]
            .drop_duplicates(["feature_id", "glycopeptide_id"])
            .rename(columns={"glycopeptide_id": "hypothesis"}),
            how="left",
        )

        index = glycoform_pp_data.groupby(["feature_id", "glycan_composition"]).apply(
            lambda x: x["posterior"].idxmax()
        )

        glycoform_pp_data["posterior"] = glycoform_pp_data.groupby(
            ["feature_id", "glycan_composition"]
        )["posterior"].transform(lambda x: np.sum(x))
    ...
    if grouped_fdr:
        glycoform_pp_data["qvalue"] = (
            pd.merge(
                glycoform_pp_data,
                transition_bm_data[["feature_id", "num_glycoforms"]].drop_duplicates(),
                on=["feature_id"],
                how="inner",
            )
        ...
        glycoform_pp_data["qvalue"] = compute_model_fdr(glycoform_pp_data["pep"])

    click.echo("Info: Storing results.")
    glycoform_data = glycoform_pp_data.rename(
        columns={"hypothesis": "glycopeptide_id"}
    ).merge(
        precursor_peakgroup_data[
            ["feature_id", "glycopeptide_id", "precursor_peakgroup_pep"]
        ...
        ["feature_id", "glycopeptide_id", "precursor_peakgroup_pep", "qvalue", "pep"]
    ]
    ...
    glycoform_data.to_sql("SCORE_GLYCOFORM", con, index=False, if_exists="replace")
    con.close()
