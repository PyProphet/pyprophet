...
def _setup_omp_threads():
    ...
    for i, arg in enumerate(argv):
        if arg == "--classifier=HistGradientBoosting":
            is_histgbc = True
            break
        elif arg == "--classifier" and i + 1 < len(argv) and argv[i + 1] == "HistGradientBoosting":
            is_histgbc = True
            break
    ...
    # Optional: print to stderr so user knows what happened
    # (can't use logger yet as it's not imported)
    print(
        f"[pyprophet] Auto-setting OMP_NUM_THREADS={omp_threads} for HistGradientBoosting "
        f"(CPUs: {total_cpus}, --threads {threads})",
        file=sys.stderr
    )
