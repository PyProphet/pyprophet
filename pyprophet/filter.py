...
def filter_osw(oswfiles, remove_decoys=True, max_gene_fdr=None, max_protein_fdr=None, max_peptide_fdr=None, max_ms2_fdr=None):
    ...
    for osw_in in oswfiles:
        ...
        if max_gene_fdr is not None and check_sqlite_table(conn, 'SCORE_GENE'):
            ...
            copy_table(c, conn, gene_ids, "GENE", "ID")
            copy_table(c, conn, gene_ids, "SCORE_GENE", "GENE_ID")
        else:
        ...
        if max_protein_fdr is not None and check_sqlite_table(conn, 'SCORE_PROTEIN'):
            ...
            # Copy filtered tables
            copy_table(c, conn, protein_ids, "PROTEIN", "ID")
            copy_table(c, conn, protein_ids, "SCORE_PROTEIN", "PROTEIN_ID")
        else:
            # Copy original full tables
        ...
        if max_peptide_fdr is not None and check_sqlite_table(conn, 'SCORE_PEPTIDE'):
            ...
            # Copy filtered tables
            copy_table(c, conn, peptide_ids, "PEPTIDE", "ID")
            copy_table(c, conn, peptide_ids, "SCORE_PEPTIDE", "PEPTIDE_ID")
            copy_table(c, conn, peptide_ids, "PRECURSOR_PEPTIDE_MAPPING", "PEPTIDE_ID")
            copy_table(c, conn, peptide_ids, "PEPTIDE_PROTEIN_MAPPING", "PEPTIDE_ID")
            copy_table(c, conn, peptide_ids, "PEPTIDE_GENE_MAPPING", "PEPTIDE_ID")
        else:
            # Copy original full tables
        ...
        if max_ms2_fdr is not None and check_sqlite_table(conn, 'SCORE_MS2'):
            ...
            # Copy filtered tables
            copy_table(c, conn, feature_ids, "FEATURE", "ID")
            if check_sqlite_table(conn, 'FEATURE_MS1'):
                copy_table(c, conn, feature_ids, "FEATURE_MS1", "FEATURE_ID")
            copy_table(c, conn, feature_ids, "FEATURE_MS2", "FEATURE_ID")
            if check_sqlite_table(conn, 'FEATURE_TRANSITION'):
                copy_table(c, conn, feature_ids, "FEATURE_TRANSITION", "FEATURE_ID")
            copy_table(c, conn, feature_ids, "SCORE_MS2", "FEATURE_ID")
            copy_table(c, conn, precursor_ids, "PRECURSOR", "ID")
        else:
            # Copy original full tables
            ...
            copy_table(c, conn, feature_ids, "FEATURE", "ID")
            if check_sqlite_table(conn, 'FEATURE_MS1'):
                copy_table(c, conn, feature_ids, "FEATURE_MS1", "ID")
            copy_table(c, conn, feature_ids, "FEATURE_MS2", "ID")
            if check_sqlite_table(conn, 'FEATURE_TRANSITION'):
                copy_table(c, conn, feature_ids, "FEATURE_TRANSITION", "FEATURE_ID")
            copy_table(c, conn, precursor_ids, "PRECURSOR", "ID")
        ...
        copy_table(c, conn, transition_ids, "TRANSITION_PRECURSOR_MAPPING", "TRANSITION_ID")
        copy_table(c, conn, transition_ids, "TRANSITION_PEPTIDE_MAPPING", "TRANSITION_ID")
        if check_sqlite_table(conn, 'SCORE_TRANSITION'):
            copy_table(c, conn, transition_ids, "SCORE_TRANSITION", "TRANSITION_ID")
        ...
        # Create Indexes
        c.execute("CREATE INDEX other.idx_feature_feature_id ON FEATURE (ID);")
        c.execute("CREATE INDEX other.idx_feature_ms1_feature_id ON FEATURE_MS1 (FEATURE_ID);")
        c.execute("CREATE INDEX other.idx_feature_ms2_feature_id ON FEATURE_MS2 (FEATURE_ID);")
        c.execute("CREATE INDEX other.idx_feature_precursor_id ON FEATURE (PRECURSOR_ID);")
        c.execute("CREATE INDEX other.idx_feature_run_id ON FEATURE (RUN_ID);")
        c.execute("CREATE INDEX other.idx_feature_transition_feature_id ON FEATURE_TRANSITION (FEATURE_ID);")
        c.execute("CREATE INDEX other.idx_feature_transition_transition_id ON FEATURE_TRANSITION (TRANSITION_ID);")
        c.execute("CREATE INDEX other.idx_peptide_peptide_id ON PEPTIDE (ID);")
        c.execute("CREATE INDEX other.idx_peptide_protein_mapping_peptide_id ON PEPTIDE_PROTEIN_MAPPING (PEPTIDE_ID);")
        c.execute("CREATE INDEX other.idx_precursor_peptide_mapping_peptide_id ON PRECURSOR_PEPTIDE_MAPPING (PEPTIDE_ID);")
        c.execute("CREATE INDEX other.idx_precursor_peptide_mapping_precursor_id ON PRECURSOR_PEPTIDE_MAPPING (PRECURSOR_ID);")
        c.execute("CREATE INDEX other.idx_precursor_precursor_id ON PRECURSOR (ID);")
        c.execute("CREATE INDEX other.idx_protein_protein_id ON PROTEIN (ID);")
        c.execute("CREATE INDEX other.idx_run_run_id ON RUN (ID);")
        c.execute("CREATE INDEX other.idx_score_ms2_feature_id ON SCORE_MS2 (FEATURE_ID);")
        c.execute("CREATE INDEX other.idx_score_protein_protein_id ON SCORE_PROTEIN (PROTEIN_ID);")
        c.execute("CREATE INDEX other.idx_score_transition_feature_id ON SCORE_TRANSITION (FEATURE_ID);")
        c.execute("CREATE INDEX other.idx_score_transition_transition_id ON SCORE_TRANSITION (TRANSITION_ID);")
        c.execute("CREATE INDEX other.idx_transition_id ON TRANSITION (ID);")
        c.execute("CREATE INDEX other.idx_transition_peptide_mapping_transition_id ON TRANSITION_PEPTIDE_MAPPING (TRANSITION_ID);")
