...
def _run_export_parquet_single_run(
    temp_folder,
    transitionLevel=False,
    pd_testing_kwargs=dict(check_dtype=False, check_names=False),
    onlyFeatures=False,
    noDecoys=False,
):
    ...
    parquet = pd.read_parquet(
        "dummyOSWScoredData.parquet"
    )  ## automatically with parquet ending of input file name
    ...
    if transitionLevel:
        if onlyFeatures:  # length of FEATURE_TRANSITION table
            if not noDecoys:
                expectedLength = len(
                    pd.read_sql("select * from feature_transition", conn)
                )
            else:
                expectedLength = len(
                    pd.read_sql(
                        "select * from feature_transition inner join transition on transition.id = feature_transition.transition_id where DECOY == 0",
                        conn,
                    )
                )
        else:
            if not noDecoys:
                featureTransition = pd.read_sql(
                    "select * from feature_transition", conn
                )
                precursorTransition = pd.read_sql(
                    "select * from transition_precursor_mapping", conn
                )
            else:
                featureTransition = pd.read_sql(
                    "select * from feature_transition inner join transition on transition.id = feature_transition.transition_id where DECOY == 0",
                ...
                precursorTransition = pd.read_sql(
                    "select * from transition_precursor_mapping inner join transition on transition.id = transition_precursor_mapping.transition_id where DECOY=0",
                    conn,
                )

            featureTable = pd.read_sql("select * from feature", conn)

            numTransNoFeature = len(
                precursorTransition[
                    ~precursorTransition["PRECURSOR_ID"].isin(
                        featureTable["PRECURSOR_ID"]
                    )
                ]
            )
        ...
        if onlyFeatures:  # expected length, length of feature table
            if noDecoys:
                expectedLength = len(
                    pd.read_sql(
                        "select * from feature inner join precursor on feature.precursor_id = precursor.id where decoy = 0",
                        conn,
                    )
                )
            else:
                expectedLength = len(
                    pd.read_sql(
                        "select * from feature inner join precursor on precursor.id = feature.precursor_id",
                        conn,
                    )
                )
        else:
            ...
            numPrecsNoFeature = len(
                precTable[~precTable["ID"].isin(featureTable["PRECURSOR_ID"])]
            )
    ...
    ]  # this is a list of columns which expect to be NAN
    parquet = parquet.drop(columns=na_columns).dropna()

    assert (
        len(parquet) > 0
    )  # assert that did not just drop everything (means that missed an na column)
    ...
    if transitionLevel:
        ...
        pd.testing.assert_series_equal(
            parquet["PRECURSOR.PRECURSOR_MZ"] // 100,
            parquet["TRANSITION.PRODUCT_MZ"] // 100,
            **pd_testing_kwargs,
        )
    ...
    pseudo_feature_id = (
        parquet["FEATURE_ID"].astype(str).str.slice(start=0, stop=1)
    ).astype(int)
    pd.testing.assert_series_equal(
        parquet["FEATURE_MS1.APEX_INTENSITY"],
        parquet["PRECURSOR_ID"],
        **pd_testing_kwargs,
    )
    pd.testing.assert_series_equal(
        parquet["FEATURE_MS2.APEX_INTENSITY"],
        parquet["PRECURSOR_ID"],
        **pd_testing_kwargs,
    )

    pd.testing.assert_series_equal(
        parquet["FEATURE_MS1.EXP_IM"],
        parquet["FEATURE_MS2.EXP_IM"],
        **pd_testing_kwargs,
    )
    pd.testing.assert_series_equal(
        parquet["FEATURE_MS2.DELTA_IM"],
        parquet["FEATURE_MS1.DELTA_IM"],
        **pd_testing_kwargs,
    )

    pd.testing.assert_series_equal(
        parquet["SCORE_MS2.SCORE"],
        (parquet["PRECURSOR_ID"] + 1)
        * parquet["FEATURE.EXP_RT"].astype(int)
        * pseudo_feature_id,
        **pd_testing_kwargs,
    )
    pd.testing.assert_series_equal(
        parquet["SCORE_PEPTIDE.SCORE_GLOBAL"],
        parquet["PEPTIDE_ID"],
        **pd_testing_kwargs,
    )
    pd.testing.assert_series_equal(
        parquet["SCORE_PROTEIN.SCORE_GLOBAL"],
        parquet["PROTEIN_ID"],
        **pd_testing_kwargs,
    )
    ...
    if transitionLevel:
        pd.testing.assert_series_equal(
            parquet["FEATURE_TRANSITION.AREA_INTENSITY"],
            parquet["TRANSITION.PRODUCT_MZ"] * pseudo_feature_id,
            **pd_testing_kwargs,
        )


def _run_export_parquet_scoring_format(
    regtest, temp_folder, compression="zstd", compression_level=11
):
    os.chdir(temp_folder)
    ...
    print(
        pd.read_parquet("test_data.parquet", engine="pyarrow")
        .head(100)
        .sort_index(axis=1),
        file=regtest,
    )
