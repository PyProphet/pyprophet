...
def test_pi0est(tmpdir):
    ...
    stat = (
        pd.read_csv("test_lfdr_ref_data.csv", delimiter=",")
        .sort_index(axis=1)
        .sort_values("p")
    )
    ...
    np.testing.assert_almost_equal(
        pi0est(stat["p"], lambda_=np.arange(0.4, 1.0, 0.05), smooth_log_pi0=True)[
            "pi0"
        ],
        0.6658949,
    )
    np.testing.assert_almost_equal(
        pi0est(stat["p"], pi0_method="bootstrap")["pi0"], 0.6763406
    )
...
def test_qvalue(tmpdir, regtest):
    ...
    stat = (
        pd.read_csv("test_qvalue_ref_data.csv", delimiter=",")
        .sort_index(axis=1)
        .sort_values("p")
    )
    ...
    # For comparison with R/bioconductor reference implementation
    np.testing.assert_almost_equal(
        qvalue(stat["p"], 0.669926026474838, pfdr=False),
        stat["q_default"].values,
        decimal=4,
    )
    np.testing.assert_almost_equal(
        qvalue(stat["p"], 0.669926026474838, pfdr=True),
        stat["q_pfdr"].values,
        decimal=4,
    )
...
def test_bw_nrd0():
    ...
    np.testing.assert_almost_equal(
        bw_nrd0(stat), 0.1736562
    )  # compared with R implementation
...
def test_stat_metrics(tmpdir, regtest):
    ...
    print(
        stat_metrics(to_one_dim_array(stat["p"]), 0.669926026474838, False).sort_index(
            axis=1
        ),
        file=regtest,
    )
    print(
        stat_metrics(to_one_dim_array(stat["p"]), 0.669926026474838, True).sort_index(
            axis=1
        ),
        file=regtest,
    )
