...
def test_0():
    test_in = {'ms1_precursor_pp': [0.6], 'ms2_peakgroup_pp': [0.8], 'ms2_precursor_pp': [0.5]}
...
def test_1():
    test_in = {'peptidoforms': ['PEPA|PEPB','PEPB|PEPC','PEPC'], 'precursor_peakgroup_pp': [0.6,0.6,0.6], 'posterior': [0.1,0.8,0.6]}
    tin = pd.DataFrame(data=test_in, index=None)
    ...
    tout = prepare_transition_bm(tin, True)
    print(tref)
    print(tout)
